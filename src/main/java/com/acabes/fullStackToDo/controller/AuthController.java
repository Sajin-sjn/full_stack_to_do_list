    ...
    public ResponseEntity<SignUpResponseDTO> signup(@Valid @RequestBody UserDTO userDTO) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body((SignUpResponseDTO) Map.of("message", "User registered successfully", "data", responseDTO, "success", true));
        } catch (BadRequestExceptions e) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body((SignUpResponseDTO) Map.of("message", e.getMessage(),"success", false));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body((SignUpResponseDTO) Map.of("message", "An unexpected error occurred","success", false));
        }
    }
}
